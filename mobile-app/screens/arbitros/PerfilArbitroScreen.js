...
const PerfilArbitroScreen = ({ navigation }) => {
  ...
  const fetchPerfil = async () => {
    try {
      ...
      const response = await fetch(API.arbitroPerfil, {
        headers: {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      const response = await fetch(API.arbitroPerfil, {
        ...
        body: JSON.stringify({ nombre: editNombre, telefono: editTelefono })
      });
    ...
    <View style={{ flex: 1, backgroundColor: COLORS.background }}>
      <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: SPACING.xxxl }}>
        <Card elevated glow style={styles.profileHeader}>
          ...
          <Text style={styles.name}>{perfil?.nombre || 'Árbitro'}</Text>
          <Text style={styles.role}>Árbitro Central / Asistente</Text>
        ...
        <View style={styles.statsRow}>
          ...
          <Card elevated style={styles.statCard}>
            ...
            <Text style={styles.statLbl}>Amarillas</Text>
          </Card>
          <Card elevated style={styles.statCard}>
            <Text style={[styles.statVal, { color: COLORS.primary }]}>0</Text>
            <Text style={styles.statLbl}>Rojas</Text>
          </Card>
        </View>
...
const styles = StyleSheet.create({
  ...
  inputLabel: { color: COLORS.textPrimary, marginBottom: SPACING.xs, fontWeight: TYPOGRAPHY.weights.bold },
  input: { 
...
export default PerfilArbitroScreen;
